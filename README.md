...
2. 首次语音：`npm run setup:tts`（装 Qwen3-TTS 到 `.deps/`）；之后 `npm run dev` 会一并起 `:8880`  
3. **默认路径**：说话或打字 → 流式回复 + **本机 TTS**（`:8000` → `:8880`）  
...
| 变量 | 用途 |
|---|---|
| `VLLM_TTS_URL` | 本机 TTS（默认 `:8880`，由 `npm run dev` / `dev:tts` 启动） |
